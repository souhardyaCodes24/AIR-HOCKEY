...
# CREDIT FONT
CRED_FONT= pygame.font.SysFont("comicsans",12,"bold")
...
def computer_moves_alg_1(player_2):
    ...
    if t%11==0 and t%1==0:
        ...
        pass
def computer_moves_alg_2(player,puck):
...
def super_alg(player,puck):
    t=random.randint(0,200)
    if t%7==0 :
        n=random.randint(0,11)
        if n%2==0:
            computer_moves_alg_1(player)
        else:
            computer_moves_alg_2(player,puck)
    else:
        pass
...
async def main():
    ...
    while run:
        ...
        #SCORE OF PLAYER 1
        score_actual=SCORE_FONT.render(f"SCORE : {int(player_2_score)}",1,score_colour1)
        w.blit(score_actual,(20,20))
        if puck.y==wall and 250<puck.x<450:
            player_2_score+=1

        # SCORE OF PLAYER 2
        score_actual_2=SCORE_FONT.render(f"SCORE : {int(player_1_score)}",1,score_colour2)
        ...
        if var<=5000:
            l=SCORE_FONT.render(f"PRESS 1 TO GRAB STRIKER | SPACE BAR TO PAUSE",1,"yellow")
            w.blit(l,(30,500))

        d=CRED_FONT.render(f"Created By: Souhardya Saha",1,"violet")
        w.blit(d,(500,20))
